...
export const IconCard: FC<IconCardProps> = ({ label, icon, iconName, imageUrl, onClick, isSelected, className }) => {
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    e.stopPropagation();
    console.log('IconCard clicked:', label);
    onClick();
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center justify-center p-4 sm:p-6 w-full h-full gap-2">
        {imageUrl ? (
          <div className="w-20 h-20 sm:w-24 sm:h-24 mb-2 relative overflow-hidden rounded-xl shadow-md">
        ...
        <p className="font-bold text-gray-800 text-sm sm:text-base md:text-lg text-center leading-tight">{label}</p>
      </div>
    </button>
  );
};
